...
freeGiftHandler = function (calledFromCartPage = false) {
    ...
    var codes = [];
    codes.push({"{{settings.gwp_code_total1total}}":"{{settings.gwp_variant_id1total}}"}); //Tiered Gift With Purchase LA-96
    codes.push({"{{settings.gwp_code_total2total}}":"{{settings.gwp_variant_id2total}}"});
    codes.push({"{{settings.gwp_code_total3total}}":"{{settings.gwp_variant_id3total}}"});
    codes.push({"{{settings.gwp_code_total4total}}":"{{settings.gwp_variant_id4total}}"});
    codes.push({"{{settings.gwp_code_total5total}}":"{{settings.gwp_variant_id5total}}"});
    ...
    codesBUYTWOGETONE.push("{{settings.gwp_variant_BTGO_id1}}".trim());
    codesBUYTWOGETONE.push("{{settings.gwp_variant_BTGO_id2}}".trim());
    codesBUYTWOGETONE.push("{{settings.gwp_variant_BTGO_id3}}".trim());
    console.log(codesBUYTWOGETONE);
    var codesBUYTWOGETONE_Groups = [];
    codesBUYTWOGETONE_Groups.push("{{settings.gwp_variant_BTGOGroup_id1}}".trim());
    codesBUYTWOGETONE_Groups.push("{{settings.gwp_variant_BTGOGroup_id2}}".trim());
    codesBUYTWOGETONE_Groups.push("{{settings.gwp_variant_BTGOGroup_id3}}".trim());
    ...
    $.getJSON('/cart.js', function(cart) {
        ...
        BUYTWOGETTHIRD("{{settings.gwp_variant_id_GWP-B2G1}}",cart,calledFromCartPage);  //LA-108 B2G1 Promotion
        console.log("BOGO NO CODE: " + "{{settings.gwp_variant_BOGO_bought}}");
        BUYONEGETONE("{{settings.gwp_variant_BOGO_bought}}","{{settings.gwp_variant_BOGO_free}}",cart,calledFromCartPage);
        BUYONEGETONE("{{settings.gwp_variant_BOGO_bought2}}","{{settings.gwp_variant_BOGO_free2}}",cart,calledFromCartPage);
...
freeGiftHandler_CheckWithDiscountCode = function (calledFromCartPage = false,cart) {
    ...
    codes.push({"{{settings.gwp_code_total1_dc}}":"{{settings.gwp_variant_id1_dc}}"}); //Tiered Gift With Purchase LA-96
    codes.push({"{{settings.gwp_code_total2_dc}}":"{{settings.gwp_variant_id2_dc}}"});
    codes.push({"{{settings.gwp_code_total3_dc}}":"{{settings.gwp_variant_id3_dc}}"});
    codes.push({"{{settings.gwp_code_total4_dc}}":"{{settings.gwp_variant_id4_dc}}"});
    codes.push({"{{settings.gwp_code_total5_dc}}":"{{settings.gwp_variant_id5_dc}}"});
    codes = codes.filter(function(obj) {
        if (Object.keys(obj)[0] == '' || obj[Object.keys(obj)[0]] == '')  //exclude empty
            return false;
        return true;
    ...
    codes2.push({"{{settings.gwp_code_total1_dc2}}":"{{settings.gwp_variant_id1_dc2}}"}); //Tiered Gift With Purchase LA-96
    codes2.push({"{{settings.gwp_code_total2_dc2}}":"{{settings.gwp_variant_id2_dc2}}"});
    codes2.push({"{{settings.gwp_code_total3_dc2}}":"{{settings.gwp_variant_id3_dc2}}"});
    codes2.push({"{{settings.gwp_code_total4_dc2}}":"{{settings.gwp_variant_id4_dc2}}"});
    codes2.push({"{{settings.gwp_code_total5_dc2}}":"{{settings.gwp_variant_id5_dc2}}"});
    ...
    var codesBUYTWOGETONE = [];
    codesBUYTWOGETONE.push("{{settings.gwp_variant_BTGO_id1_dc}}".trim());
    codesBUYTWOGETONE.push("{{settings.gwp_variant_BTGO_id2_dc}}".trim());
    codesBUYTWOGETONE.push("{{settings.gwp_variant_BTGO_id3_dc}}".trim());

    var codesBUYTWOGETONE_Groups = [];
    codesBUYTWOGETONE_Groups.push("{{settings.gwp_variant_BTGOGroup_id1_dc}}".trim());
    codesBUYTWOGETONE_Groups.push("{{settings.gwp_variant_BTGOGroup_id2_dc}}".trim());
    codesBUYTWOGETONE_Groups.push("{{settings.gwp_variant_BTGOGroup_id3_dc}}".trim());
    ...
    TIERED_BY_TOTAL(codes,cart,calledFromCartPage,"{{settings.gwp_code_code_dc}}");   //LA-96 Tiered GWP-by Cart Total
    TIERED_BY_TOTAL(codes2,cart,calledFromCartPage,"{{settings.gwp_code_code_dc2}}");   //LA-96 Tiered GWP-by Cart Total
    //now do GWP-buy two, get third free
    ...
    BUYONEGETONE("{{settings.gwp_variant_BOGO_bought_dc}}","{{settings.gwp_variant_BOGO_free_dc}}",cart,calledFromCartPage,"{{settings.gwp_code_code_BOGO_dc}}");
    BUYONEGETONE("{{settings.gwp_variant_BOGO_bought_dc_percent}}","{{settings.gwp_variant_BOGO_free_dc_percent}}",cart,calledFromCartPage,"{{settings.gwp_code_code_BOGO_dc_percent}}","{{settings.gwp_amount_BOGO_free_dc_percent}}",true);
    CheckReload();
    //BUYTWOGETTHIRD("{{settings.gwp_variant_id_GWP-B2G1-DiscountCode}}",cart,calledFromCartPage,"{{gwp_codeGWP-B2G1-DiscountCode}}");
...
function BUYTWOGETONE_Groups(codesBUYTWOGETONE_Groups,cart,calledFromCartPage,discount_code=0)
{
    var thisDC = "";
    if (discount_code == 1)
        thisDC = "{{settings.gwp_code_code_BTGO_dc}}";
    else if (discount_code == 2)
        thisDC = "{{settings.gwp_code_code_BTGOGroup_dc}}";
...
var codeSimple = "{{settings.gwp_code}}";
var currentCode = "";
/*
    ...
    addGWP = function (cart) {
      ...
      if (!found)
        ...
        if ($('tr.product[data-variant-id={{settings.gwp_variant_id}}] td.product__price span').length == 0)
          return;

        var value = $('tr.product[data-variant-id={{settings.gwp_variant_id}}] td.product__price span').html();
        if (value == '$0.00' || value.toLowerCase() == 'free' || value.indexOf('strike') > -1)
          return;

        value = '<span class="strike">' + value + "</span>  $0.00";
        $('tr.product[data-variant-id={{settings.gwp_variant_id}}] td.product__price span').html(value);
      }
    };
